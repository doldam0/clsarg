...
from typing import (
    Any, Callable, Dict, Iterable, List, Literal, Optional, Tuple, Type,
    TypeVar, Union, get_args, get_origin, get_type_hints, overload
)
...
class ArgumentParser:
    ...
    def __init__(self,
        ...
        for name in dir(self.__class__):
            ...
            if isinstance(item, property) and \
                    hasattr(item.fget, '__argument_info'):
                ...
                if item.fget is not None:
                    ...
                    arguments.append((number, (
                        argument_name, args, kwargs
                    )))

        arguments = sorted(arguments, key=lambda x: x[0])
        for argument_name, args, kwargs in [elem[1] for elem in arguments]:
            parser.add_argument(
                f'--{argument_name}', *args, **kwargs
            )
...
class _GenerateArgumentGetter:
    ...
    def __call__(
        ...
        **__kwargs: Any,
        ...
        __kwargs['required'] = True

        if getter.__doc__ is not None:
            __kwargs['help'] = getter.__doc__

        argument_type = get_type_hints(getter).get('value')
        wrapper = None

        if __kwargs.get('const', False):
            ...
            __kwargs['required'] = False
            __kwargs['action'] = 'store_const'
        elif argument_type is None:
            raise ValueError("An argument function should have value "
                             "parameter.")
        ...
        if argument_type == bool:
            __kwargs['required'] = False
            __kwargs['action'] = 'store_true'
            argument_type = None
        elif origin_type == list:
            if len(type_args) > 1:
                raise TypeError("You should set the return type of argument "
                                "function to list that contains only one "
                                "type.")
            __kwargs['nargs'] = __kwargs.get('nargs', '*')
        ...
        if argument_type is not None:
            ...
            if sig.default != Parameter.empty:
                __kwargs['required'] = False
                __kwargs['default'] = sig.default
            __kwargs['type'] = argument_type

        if wrapper is None:
            def __wrapper(self: ArgumentParser):
                return getter(self, getattr(
                    self.arguments, argument_name
                ))
            wrapper = __wrapper

        setattr(wrapper, '__argument_info', {
...
def argument(
    ...
    Args:
        getter (Callable[[Any, ArgumentType[T]], ReturnType]):
            An argument getter function.
        ...
        nargs (int | '?' | '*' | '+', optional): Defaults to None.
